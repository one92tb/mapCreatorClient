          ...
          <Tbody>
            {
              users.filter(user => user.login.search(login) !== -1 && user).map((user, id) => {
                ...
                return (<Tr key={user.id} data-testid="user">
                  <Td>{id + 1}</Td>
                  <Td>{user.login}</Td>
                  <Td>
                    {
...
Users.propTypes = {
  users: PropTypes.arrayOf(PropTypes.shape({id: PropTypes.number.isRequired, login: PropTypes.string.isRequired, password: PropTypes.string.isRequired, isAdmin: PropTypes.bool.isRequired})),
