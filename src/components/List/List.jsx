          ...
          <Tbody>
            {
              indicators.filter(indicator => {
                return (markerName === "All" && city === "") || (markerName === "All" && indicator.city.toLowerCase().search(city.toLowerCase()) !== -1)
                  ? indicator
                  : markerName === indicator.name && indicator.city.toLowerCase().search(city.toLowerCase()) !== -1 && indicator;

                //1 ) sprawdza wszystko ALL ""
                // 2) All  Wroclaw
                // 3 ) test1 Wroclaw
              }).map((indicator, id) => (<Tr key={indicator.id} data-testid="indicator">
                <Td>{id + 1}</Td>
